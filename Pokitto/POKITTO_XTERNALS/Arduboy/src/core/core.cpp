#include "core.h"
#include "Pokitto.h"
#include "pokitto_fakeavr.h" // jonne
#include "binary.h" // jonne
...
void ArduboyCore::paintScreen(const unsigned char *image)
{
  ...
  {
    //SPI.transfer(pgm_read_byte(image + i)); // jonnetodo
  }
}
...
{
  _ac.display.lcdRefresh((uint8_t*)image); // jonne
  //_c.display.update();
  /* jonnetodo
  for (int i = 0; i < (HEIGHT*WIDTH)/8; i++)
  {
...
uint8_t ArduboyCore::buttonsState()
{
  ...
  // jonne added this
  #ifdef POK_SIM
    _ac.buttons.pollButtons();
  #endif // POK_SIM
  if (_ac.downBtn()) buttons |= DOWN_BUTTON;
  if (_ac.upBtn()) buttons |= UP_BUTTON;
  if (_ac.leftBtn()) buttons |= LEFT_BUTTON;
  if (_ac.rightBtn()) buttons |= RIGHT_BUTTON;
  if (_ac.aBtn()) buttons |= A_BUTTON;
  if (_ac.bBtn()) buttons |= B_BUTTON;
/* jonnetodo

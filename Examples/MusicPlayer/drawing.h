...
//    if(numCols-from > 255)return;
    for (int i=from; i<=numCols; i++) mygame.display.palette[i] = p[i];
    mygame.display.paletteptr = mygame.display.palette;
}

void load4bitPalette(const uint16_t* p, uint8_t from, uint8_t numCols) {
//    if(numCols-from > 15)return;
    for (int i=from; i<numCols; i++) mygame.display.palette[i+256] = p[i];
    mygame.display.paletteptr = mygame.display.palette;
}

void load2bitPalette(const uint16_t* p, uint8_t from, uint8_t numCols) {
//    if(numCols-from > 3)return;
    for (int i=from; i<numCols; i++) mygame.display.palette[i+272] = p[i];
    mygame.display.paletteptr = mygame.display.palette;
}
...
void drawPixel8(uint16_t x, uint16_t y, uint16_t col){
    ...
    if(col == mygame.display.invisiblecolor) return;
    mygame.display.screenbuffer[x+110*y]=col;
}
...
void drawPixel4(uint16_t x, uint16_t y, uint16_t col){
    ...
    uint16_t i = y*55 + (x>>1);
    uint16_t pixel = mygame.display.screenbuffer[i];
    if (x&1) pixel = (pixel & B11110000)|(col);
        else pixel = (pixel & B00001111) | (col<<4);
    mygame.display.screenbuffer[i] = pixel;
}

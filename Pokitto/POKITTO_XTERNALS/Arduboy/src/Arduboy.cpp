...
#include "glcdfont.c"
#include "ab_logo.c"
#include "PokittoCore.h" // jonne

#define min(a,b) ((a)<(b)?(a):(b)) // jonne
#define max(a,b) ((a)>(b)?(a):(b)) // jonne
...
{
  long now = pokGetTime(); //long now = millis(); // replaced with pokitto service - jonne
...
{
  //power_adc_enable(); // ADC on // jonne
  // randomSeed(~rawADC(ADC_TEMP) * ~rawADC(ADC_VOLTAGE) * ~micros() + micros()); // jonne
  // power_adc_disable(); // ADC off // jonne
}
...
{
  int end = y+h;
  for (int a = __avrmax(0,y); a < __avrmin(end,HEIGHT); a++)
  {
    drawPixel(x,a,color);
  }
}
...
void Arduboy::fillScreen(uint8_t color)
{
  // C version :
  //
  if (color) color = 0xFF;  //change any nonzero argument to b11111111 and insert into screen array. //enabled this jonne
  for(int16_t i=0; i<1024*POK_COLORDEPTH; i++)  { sBuffer[i] = color; }  //sBuffer = (128*64) = 8192/8 = 1024 bytes. //enabbled this jonne
...
(int16_t x, int16_t y, unsigned char c, uint8_t color, uint8_t bg, uint8_t size)
{
  ...
  for (int8_t i=0; i<6; i++ )
  {
    ...
    {
      line = pgm_read_byte(ardufont+(c*5)+i); // changed for to ardufont - jonne
    }

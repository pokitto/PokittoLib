...
struct Shape
{
  ...
  virtual void Draw(PokittoApp* pa) const = 0;
...
struct Circle : public Shape
{
  ...
  void Draw(PokittoApp* _po) const
  {
...
struct PolygonShape : public Shape
{
  ...
  void Draw(PokittoApp* _po) const
  {
    ...
    //glBegin( GL_LINE_LOOP );
    Vec2 v,v2;
    v = body->position + u * m_vertices[0];
    for(uint32 i = 0; i < m_vertexCount-1; ++i)
    {
      v2 = body->position + u * m_vertices[i+1];
      _po->drawLine(v.x,v.y,v2.x,v2.y);
      v=v2;
      //glVertex2f( v.x, v.y );
    }
    v2 = body->position + u * m_vertices[0];
    _po->drawLine(v.x,v.y,v2.x,v2.y); // draw last vertex
    //glEnd( );
  }

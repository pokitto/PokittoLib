...
int getWidth();
int getHeight();
byte tileAtPosition(int,int);
boolean solidCollisionAtPosition(int,int,int,int);
void draw();
void chooseMap();
boolean addScore(int);
byte getType();
int getWidth();
int getHeight();
int getGravity();
    ...
    void chooseMap() {
      ...
      while (1) {
        if (gb.update()) {
          ...
          char dumbo[17] = "\21 Select map \20";
          printCentered((char*)dumbo);
...
void printCentered(const char* text) {
  gb.display.cursorX = (LCDWIDTH / 2) - (strlen((const char*)text) * gb.display.fontSize * gb.display.fontWidth / 2);
...
void updatePopup() {
  if (popupTimeLeft) {
    ...
    byte width = strlen((const char*) popupText) * gb.display.fontSize * gb.display.fontWidth;
...
/*void adjustContrasts(){
  ...
  while(1){
    if(gb.update()){
      ...
      gb.display.cursorY = 0;
      gb.display.setColor(BLACK, WHITE);
      gb.display.print(("Adjust contrasts\nfor this game:  "));
      gb.display.print(("\37"));
      gb.display.print(contrast);
      gb.display.println(("\36"));
      gb.display.cursorY = LCDHEIGHT - 5;
      gb.display.print(("  A:OK B:Default"));
    }
  }
}*/
